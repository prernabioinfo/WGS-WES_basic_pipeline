...
def step_select_snps(args, dirs, base, vcf, master):
    master.info("  ── STEP 10 : GATK SelectVariants + SNP hard-filter")
    log = step_logger(dirs["logs"] / "step10_select_filter_snps.log")
    filt_vcf = f"{dirs['vc']}/{base}_filtered_SNPs.vcf.gz"
    run(
        f"{args.gatk} --java-options \"-Xmx{args.java_mem}\" SelectVariants "
        f"-R {args.genome} -V {vcf} --select-type-to-include SNP -O {filt_vcf} "
    ...
    master.info("     log → %s", dirs['logs'] / 'step10_select_filter_snps.log')
    return filt_vcf


def step_pass_snps(args, dirs, base, filt_vcf, master):
    master.info("  ── STEP 11 : GATK SelectVariants — extract PASS SNPs")
    log = step_logger(dirs["logs"] / "step11_pass_snps.log")
    pass_vcf = f"{dirs['vc']}/{base}_SNPs_final_PASS.vcf.gz"
    run(
        f"{args.gatk} --java-options \"-Xmx{args.java_mem}\" SelectVariants "
        f"-V {filt_vcf} --exclude-filtered -O {pass_vcf}",
        log, args.dry_run,
    )
    master.info("     log → %s", dirs['logs'] / 'step11_pass_snps.log')
    return pass_vcf
...
def run_sample(args, sample: dict, master: logging.Logger) -> bool:
    ...
    try:
        ...
        if not args.skip_vc:
            ...
            filt_vcf = step_select_snps(args, dirs, base, vcf, master)
...
def main() -> None:
    ...
    for i, sample in enumerate(samples, 1):
        ...
        sa      = sample_args(args, sample)   # per-sample RG overrides
        success = run_sample(sa, sample, master)
    ...
    master.info("")
    master.info("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    master.info("PIPELINE SUMMARY")
    master.info("  Total    : %d", total)
    master.info("  Passed   : %d  %s", len(passed), passed)
    master.info("  Failed   : %d  %s", len(failed), failed)
    master.info("  Elapsed  : %s", str(elapsed).split(".")[0])
